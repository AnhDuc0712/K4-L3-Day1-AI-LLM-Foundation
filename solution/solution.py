...
def call_openai(
    ...
    start_time = time.perf_counter()  # SỬA: Dùng perf_counter thay vì time.time()
    ...
    latency_seconds = time.perf_counter() - start_time  # SỬA
    
    return response.choices[0].message.content, latency_seconds
...
def compare_models(prompt: str) -> dict:
    gpt4o_text, gpt4o_lat = call_openai(prompt, model=OPENAI_MODEL)
    mini_text, mini_lat = call_openai_mini(prompt)
    
    # SỬA: Dùng estimate_cost (đã định nghĩa ở Part 2) và trả về 5 key phẳng
    gpt4o_cost = estimate_cost(prompt, gpt4o_text, OPENAI_MODEL)["total_cost"]
...
def streaming_chatbot() -> None:
    ...
    print("Chatbot (gõ 'quit' hoặc 'exit' để thoát)")
    while True:
        ...
        messages = [{"role": "system", "content": "Bạn là trợ lý hữu ích."}] + history + [{"role": "user", "content": user_input}]
...
def run_assistant(
    ...
    while True:
        ...
        cost_dict = estimate_cost(user_msg, reply, OPENAI_MODEL)
